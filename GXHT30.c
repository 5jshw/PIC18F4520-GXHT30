...
extern float Temperature;	//外部变量：温度
extern float Humidity;		//外部变量：湿度
...
unsigned char IIC_ReadByte(unsigned int ack)	//ack 判断字节数据是否接收完成
{
   ...
   for(i = 0; i < 8; i++)
   {
		...
		while(!SCL);					//判断时钟信号
		receive <<= 1;					//数据存储变量向左移动一位
		if(SDA == 1)					//判断数据接口收到的数据
		{
			receive++;					//相同则加一
		}
   }
   
   	if(ack == 0)						//判断调用该函数时的ack的值，也即是否接受完数据，是否发送结束信号
	{
